...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  let sale = await prisma.storeSale.findFirst({
    where: { id, businessId: session.businessId },
    include: { business: true, storeItem: true, soldBy: true },
  });
  ...
  if (sale.receiptNumber === null) {
    ...
    sale = await prisma.storeSale.update({
      where: { id: sale.id },
      data: { receiptNumber },
      include: { business: true, storeItem: true, soldBy: true },
    });
  ...
  doc.text(`${sale.storeItem.name} x${sale.quantity}`, 40, doc.y, { continued: true, width: 250 });
